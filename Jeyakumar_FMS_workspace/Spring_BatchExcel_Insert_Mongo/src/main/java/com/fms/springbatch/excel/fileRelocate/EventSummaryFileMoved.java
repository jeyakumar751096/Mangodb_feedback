...
@Component
public class EventSummaryFileMoved implements Tasklet {

	Logger log = LoggerFactory.getLogger(EventSummaryFileMoved.class);
	
	@Autowired
	private OutReachEventSummaryXLFileProperties eventSummaryFileProperties;

	private boolean moveProcessedFile() {
		FileSystemResource source = new FileSystemResource(eventSummaryFileProperties.getFileLocation());
		FileSystemResource destination = new FileSystemResource(eventSummaryFileProperties.getOutputFileLocation());
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		log.info("EventSummaryFileMoved file moved..?");
		boolean moved =false;
		if (eventSummaryFileProperties.getShouldMoveFile())
			moved =	moveProcessedFile();
		log.info("EventSummaryFileMoved file moved..?"+moved);
